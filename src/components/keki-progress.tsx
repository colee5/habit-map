...
export default function KekiProgress() {
  ...
  const handleAddActivity = async () => {
    if (otp === '252525' && selectedActivity) {
      await addActivity(selectedActivity, new Date(), 1, 'keki');
  ...
  return (
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 lg:p-20 gap-8 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-8 row-start-2 items-start max-w-full overflow-scroll">
        ...
        <HeatmapStudy year={year} user="keki" refresh={refreshTrigger} />
        <HeatmapWorkout year={year} user="keki" refresh={refreshTrigger} />
      </main>
    </div>
  );
}
